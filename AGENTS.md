...
4. **Tests are part of done.** Every ticket includes writing tests AND making
   them pass. `cd backend && PYTHONPATH=. py -m pytest` must exit 0 before you stop.
...
- **backend-test** (pytest in `backend/`; coverage printed) — BLOCKING on test FAILURES
- **backend-lint** (`ruff check` in `backend/`) — BLOCKING
- **frontend-build** (`npm ci` + `npm test` + `npm run build` in `frontend/`) — BLOCKING

Note the shape: **three jobs, and each one skips itself when its directory is absent
from your branch.** A backend-only ticket does not run frontend-build at all. But a
ticket that touches `frontend/` DOES, and it is as merge-blocking as pytest is.

Important consequences for you:
1. **`cd backend && PYTHONPATH=. py -m pytest` must be green** if your ticket
   touches `backend/`. A failing or non-collecting test kills the CI. There is NO
   coverage threshold — do not write tests against stubs to raise a percentage.
2. **Run `ruff format .` and `ruff check --fix .` from inside `backend/`** — that is
   the directory the pipeline runs them in.
3. **If your ticket touches `frontend/`: `npm run build` must succeed**, and if
   `package.json` has a `test` script, `npm test` must pass too. Both run in the
   pipeline and both block the merge. Run them yourself in `frontend/` before you
   finish — they terminate on their own, unlike `npm run dev`, which never returns.
4. **Type hints are expected** on new Python functions (argument types + return type).
5. **No hardcoded secrets/keys/URLs** — use env vars.
6. **`backend/requirements.txt` must be up to date.** New libs → add them, with a
   version bound.
7. **`package-lock.json` and `package.json` must be committed together.**
   The pipeline installs the frontend with `npm ci`, which does not resolve anything
   — it replays the lockfile and refuses if the two files disagree. So the moment you
   add, remove or re-range a dependency, re-run `npm install` in `frontend/` and
   commit the regenerated lockfile in the SAME change. Adding a devDependency and
   leaving the lockfile alone is the classic form: everything works here because your
   `node_modules` is already populated, and the pipeline dies before the first test
   with `npm error Missing: <pkg> from lock file`. Never hand-edit the lockfile, and
   never delete it to make an error go away — a frontend without one installs
   unpinned versions on every run.
